...
class optStruct:
    ...
    def __init__(self, dataMatIn, classLabels, C, toler, kTup):
        ...
        for i in range(self.m):  # 这里提前计算所有数据的核K，每个向量都有和所有余下向量，包括自己100个向量两两求内积，即100*100个内积（核）
...
def kernelTrans(X, A, kTup):
    ...
    if kTup[0] == 'lin':
        ...
        for j in range(m):  # 遍历每一个元素  内层循环
            deltaRow = X[j, :] - A
            K[j] = deltaRow * deltaRow.T  # 分子的平方
...
def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup=('lin', 0)):
    ...
    while (iter < maxIter) and ((alphaPairsChanged > 0) or (entireSet)):  # 遍历整个数据集都alpha也没有更新或者超过最大迭代次数,则退出循环
        ...
        if entireSet:  # 遍历整个数据集
            for i in range(oS.m):
                alphaPairsChanged += innerL(i, oS)  # 使用优化的SMO算法 传一行进去
...
def testRbf(k1=1.3):
    ...
    # 实际上下面几步就是把阿尔法大于0的筛选出来，带入超平面公式计算
    svInd = np.nonzero(alphas.A > 0)[0]  # 获得支持向量索引
    sVs = datMat[svInd]  # svs储存支持向量
    labelSV = labelMat[svInd]  # 支持向量（点）对应的标签
